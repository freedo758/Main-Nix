...
in
{
  ...
  xdg.configFile."matugen/config.toml".text = ''
    ...
    post_hook = "${config.xdg.configHome}/matugen/post-hook-scripts/gtk-themes-reload.sh {{mode}}"
...
[templates.caelestia]
    input_path =  "${templates}/quickshell-colors.json"
    output_path = "/home/leo/.local/state/caelestia/scheme.json"
  '';
  ...
  # Forces GTK4/libadwaita apps (and nwg-look, which reads gsettings directly,
  # not settings.ini) to pick up the new color-scheme after a matugen run.
  #
  # This used to live unmanaged at ~/.config/matugen/post-hook-scripts/ and
  # had a real bug: it toggled color-scheme based on whatever gsettings
  # *already* held, never on matugen's actual new mode. That meant the final
  # value was anchored to stale state - if gsettings said prefer-dark from a
  # previous run, it stayed on prefer-dark even after switching to a light
  # wallpaper, so nwg-look/libadwaita apps silently drifted out of sync with
  # colors.css. Now [templates.gtk4]'s post_hook passes {{mode}} in as $1, so
  # the target value is always matugen's actual mode; the double-set is kept
  # only to guarantee the dconf change signal fires (GNOME apps don't re-read
  # a key that's set to the value it already has).
  xdg.configFile."matugen/post-hook-scripts/gtk-themes-reload.sh" = {
    executable = true;
    text = ''
      #!/usr/bin/env bash
      # $1 = matugen's mode, "dark" or "light"
      mode="''${1:-dark}"
      target="prefer-$mode"
